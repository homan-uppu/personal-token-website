...
Personal tokens exist within a network (the "Network"). Every user, whether they want to raise capital, or invest in other personal tokens, will need to create their personal token ([apply here](/todo) for early access. As of now, we only accept individuals based out of the US without a criminal history.).
